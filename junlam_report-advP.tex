...
% 과학전람회 양식에 맞추어 수정하였음.
\include{subP/preamble} % usepackage 등의 명령어는 여기에.
\usepackage{cite}
\usepackage{textcomp}
\usepackage{tocloft}
...
\begin{document}
	...
	\include{subP/abstract} % Abstract
	...
	\include{subP/1_introduction} % 서론
	\include{subP/2_theoritical_background} % 이론적 배경
	\include{subP/3_method_of_the_study} % 연구 과정
	\include{subP/4_result_and_discussion} % 결과 및 토의
	%\include{subP/5_conclusion} % 결론
        \include{subP/6_further_study} % 추후 연구
	\include{subP/7_appendix} % 부록
